...
IF (opr(1)%hi == 0.0_r10 .AND. opi(1)%hi == 0.0_r10) THEN
  fail = .true.
  RETURN
END IF
...
10 IF (opr(nn)%hi == 0.0_r10 .AND. opi(nn)%hi == 0.0_r10) THEN
  idnn2 = degree - nn + 2
  zeror(idnn2) = 0.0_r10
  zeroi(idnn2) = 0.0_r10
  nn = nn - 1
...
END DO
DO  jj = 1, l1
  IF (cmod(hr(n), hi(n)) > eta*10.0_r10*cmod(pr(n), ci(n))) THEN
...
! IS THE LAST H POLYNOMIAL.
  IF (.NOT.(bool.OR..NOT.test.OR.j == l2)) THEN
    IF (cmod(tr-otr,ti-oti) < .5_r10*cmod(zr,zi)) THEN
      IF (pasd) THEN
...
! EVALUATE P AT S AND TEST FOR CONVERGENCE.
  ...
  mp = cmod(pvr, pvi)
  ms = cmod(sr,si)
  IF (mp <= 20.0_r10*errev(nn, qpr, qpi, ms, mp, are, mre)) THEN
...
! IN EVALUATING P, TERMINATE THE ITERATION.
  ...
  END IF
  IF (i /= 1) THEN
    IF (.NOT.(b .OR. mp < omp .OR. relstp >= quad(.05_r10,0.0_r10))) THEN
...
! SHIFT STEPS INTO THE CLUSTER TO FORCE ONE ZERO TO DOMINATE.
      ...
      r1 = SQRT(tp)
      r2 = sr * (1.0_r10+r1) - si * r1
      si = sr * r1 + si * (1.0_r10+r1)
      sr = r2
      CALL polyev(nn, sr, si, pr, ci, qpr, qpi, pvr, pvi)
      DO  j = 1, 5
        CALL calct(bool)
...
bool = cmod(hvr,hvi) <= are * 10.0_r10 * cmod(hr(n), hi(n))
IF (.NOT.bool) THEN
...
30 IF (ABS(dx/x) > quad(.005_r10,0.0_r10)) THEN
  q(1) = pt(1)
  DO  i = 2, nn
    q(i) = q(i-1) * x + pt(i)
  END DO
...
DO  i = 1, nn
  x = pt(i)
  IF (x > MAX) MAX = x
  IF (x%hi /= 0.0_r10 .AND. x < MIN) MIN = x
END DO
...
x = low / MIN
IF (x%hi <= 1.0_r10) THEN
  sc = 1.0_r10 / (SQRT(MAX)*SQRT(MIN))
ELSE
  sc = x
  IF (infin/sc > MAX) sc = 1.0_r10
END IF
l = LOG(sc) / LOG(base) + .5
fn_val = base ** l
...
IF (br%hi == 0.0_r10 .AND. bi%hi == 0.0_r10) THEN
...
ai = ABS(i)
IF (ar < ai) THEN
  fn_val = ai * SQRT(1.0_r10 + (ar/ai)**2)
  RETURN
END IF
IF (ar > ai) THEN
  fn_val = ar * SQRT(1.0_r10 + (ai/ar)**2)
  RETURN
END IF
fn_val = ar * SQRT(2.0_r10)
...
base   = RADIX(0.0_r10)
eta    = EPSILON(0.0_r10) * 1.0E-15_r10
infiny = HUGE(0.0_r10)
smalno = TINY(0.0_r10)
...
p(2) = -1.998046875_r10
p(3) = 0.0
p(4) = .7567065954208374_r10
p(5) = -.2002119533717632_r10
p(6) = 1.271507365163416E-2_r10
p(7) = 0
p(8) = -1.154642632172909E-5_r10
p(9) = 1.584803612786345E-7_r10
p(10) = -4.652065399568528E-10_r10
p(11) = 0
ci(1) = 0
ci(2) = p(2)
ci(3) = 2.658859252929688_r10
ci(4) = -7.567065954208374E-1_r10
ci(5) = 0
ci(6) = p(6)
ci(7) = -7.820779428584501E-4_r10
ci(8) = -p(8)
ci(9) = 0
ci(10) = p(10)
ci(11) = 9.094947017729282E-13_r10
CALL prtc(11, p, ci)
CALL cpoly(p, ci, 10, zr, zi, fail)
IF (fail) GO TO 80
CALL prtz(10, zr, zi)
