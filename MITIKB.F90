...
ELSE IF (x < 5.0D0) THEN
  ...
  ti = ((((((((.59434D-3*t+.4500642D-2)*t+.044686921D0)*t+  &
      .300704878D0)*t+1.471860153D0)*t+4.844024624D0)*t+  &
      9.765629849D0)*t+10.416666367D0)*t+5.0D0) * t1
ELSE IF (x >= 5.0 .AND. x <= 8.0D0) THEN
  t = 5.0D0 / x
  ti = (((-.015166D0*t - .0202292D0)*t + .1294122D0)*t - .0302912D0) * t  &
       + .4161224D0
  ti = ti * EXP(x) / SQRT(x)
ELSE
  t = 8.0D0 / x
  ti = (((((-.0073995D0*t + .017744D0)*t - .0114858D0)*t + .55956D-2)*t +  &
       .59191D-2)*t + .0311734D0) * t + .3989423D0
  ti = ti * EXP(x) / SQRT(x)
END IF
IF (x == 0.0D0) THEN
  tk = 0.0D0
...
ELSE IF (x > 2.0 .AND. x <= 4.0D0) THEN
  t = 2.0D0 / x
  tk = (((.0160395D0*t-.0781715D0)*t + .185984D0)*t-.3584641D0) * t  &
      + 1.2494934D0
  tk = pi / 2.0D0 - tk * EXP(-x) / SQRT(x)
ELSE IF (x > 4.0 .AND. x <= 7.0D0) THEN
  t = 4.0D0 / x
  tk = (((((.37128D-2*t-.0158449D0)*t + .0320504D0)*t-.0481455D0)*t +   &
      .0787284D0)*t-.1958273D0) * t + 1.2533141D0
  tk = pi / 2.0D0 - tk * EXP(-x) / SQRT(x)
ELSE
  t = 7.0D0 / x
  tk = (((((.33934D-3*t-.163271D-2)*t + .417454D-2)*t-.933944D-2)*t +   &
      .02576646D0)*t-.11190289D0) * t + 1.25331414D0
  tk = pi / 2.0D0 - tk * EXP(-x) / SQRT(x)
END IF
RETURN
END SUBROUTINE itikb
 
END MODULE itikb_func
