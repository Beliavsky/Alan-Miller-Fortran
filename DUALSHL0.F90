...
IF (im > 0) THEN
  IF(im <= 5) THEN
    DO j=1,15
      IF(j <= 5)  THEN
        g(j)=g(j) - 2.d0*c(j,im)*penalty
      ELSE
        g(j)=g(j) + a(j-5,im)*penalty
      END IF
    END DO
    g(im)=g(im) - 6.d0*d(im)*x(im)*penalty
  ELSE
    g(im-5)=g(im-5) - penalty
  END IF
END IF
