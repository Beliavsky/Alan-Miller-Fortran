MODULE bounded_nonlin_LS
  ...
  ! Revised on August 14, 2014 by Jason R. Blevins to provide explicit
  ! interface for ffunc.  See http://jblevins.org/log/elsunc-interface
  ! for details.
...
CONTAINS
  ...
  SUBROUTINE elsunc(x,n,mdc,m,ffunc,bnd,bl,bu, p,w, EXIT,f,c)
    ...
    REAL (dp), INTENT(IN OUT)  :: bu(:)
    INTEGER, INTENT(OUT)       :: p(11)
...
50  restar=.false.
  ...
  SUBROUTINE gnavuc(k,restar,kod,secind,fsum,beta,prekm1,nract,aset,gndok)
    ...
    ac=MIN(one,aupkm1)
    IF(pgress > c2*ac*(2.0D0-ac)*d1km1 .AND. fnorm < c3*beta) return
...
70  bestpg=0.5D0*fsum - philat
    irank=lattry
80  pgress=0.5D0*fsum - philat
...
100 RETURN
  ...
  SUBROUTINE pregn(s,sn,b,bn,mindim,prank,dim)
    ...
    REAL (dp), INTENT(IN OUT)  :: sn
    REAL (dp), INTENT(IN OUT)  :: b(:)
    REAL (dp), INTENT(IN OUT)  :: bn
    ...
    DO  k=1,n
       ...
       DO  j=1,k
          ...
          DO  l=1,m
             ...
             sum=sum + term/(4.0D0*epsk*epsj)*v(l)
          END DO
          b(k,j)=sum
          IF(k == j) CYCLE
          b(j,k)=sum
...
60  IF(k > maxit) EXIT=-2
  ...
  SUBROUTINE stepuc(x,g,dx,n,f,v1,m,phi,ffunc,kod,prank,bl,bu,  &
    ...
    alphup=3.0D0
    !************************
    IF(ifree > 0 .AND. indic < 0) alphup=10.0D0*alphup
    !************************
    IF(bnd /= 0)THEN
...
40  alpha=MIN(one,alphup)
    ...
    IF(-1 == ctrl) phi=phi*2.0D0
    IF(ctrl == 1) phi=0.5D0*dnrm2(m,f,1)**2
...
40  error=ctrl
  ...
  SUBROUTINE reavuc(error,alpha,alphup,fsum,phi,restar)
    ...
    IF(-2 == error) GO TO 10
    IF(phi < 0.5D0*fsum) GO TO 10

5   error=-5
    RETURN

10  IF(alpha <= alphup/3000.0D0) restar=.true.
  ...
  SUBROUTINE outuc (iprint, k, restar, UNIT, phi, gnorm, eval, aset,  &
    ...
    reduc=fsqkm1 - 2.0D0*phi
    ac=MIN(one,aupkm1)
    pred=ac*(2.0D0-ac)*d1km1
...
20  EXIT=-1
  ...
  SUBROUTINE minrm(v0,v1,v2,m,alfmin,alfmax,xmin,x,px,y,py)
    ...
    pbiss=xmin**2*v2norm*v2norm*6.0D0 + xmin*scv1v2*6.0D0 + 2.0D0*scv0v2 + &
         v1norm**2
    h0=ABS(pprim/pbiss)
    dm=ABS(6.d0*scv1v2 + xmin*v2norm*v2norm*12.0D0) + h0*v2norm*v2norm*24.0d0
...
50  pprim=pol3(scv0v1,scv0v2,v1norm,scv1v2,v2norm,x0)
    pbiss=x0**2*v2norm*v2norm*6.0D0 + x0*scv1v2*6.0D0 + 2.0D0*scv0v2 + v1norm**2
    d=-pprim/pbiss
    x=x0 + d
...
100 x=MIN(x,alfmax)
  ...
  SUBROUTINE minrm2(v1nrm,v2nrm,scv0v1,scv0v2,scv1v2,p,q,delta,a1div3)
    ...
    a1=1.5D0*scv1v2/v2nrm/v2nrm
    a2=0.5D0*((v1nrm/v2nrm)**2 + scv0v2/v2nrm/v2nrm*2.0D0)
    a3=0.5D0*scv0v1/v2nrm/v2nrm
    p=a2 - a1*a1/3._dp
  ...
  SUBROUTINE twor(p,q,delta,a1div3,x1,x2,x3)
    ...
    sqd=SQRT(-delta)
    IF(ABS(q) <= 2.0D0*eps*sqd) THEN
       fi=ATAN(one)*2.0D0
    ELSE
       tanfi=ABS(2.0D0*sqd/q)
       fi=ATAN(tanfi)
    END IF
  ...
  FUNCTION pol3(scv0v1,scv0v2,v1nrm,scv1v2,v2nrm,x) RESULT(fn_val)
    ...
    fn_val=scv0v1 + x*v1nrm*v1nrm + 2.0D0*x*scv0v2 + x**2*3.0D0*scv1v2 +  &
         x**3*v2nrm*v2nrm*2.0D0
  ...
  SUBROUTINE fsumsq(xold,p,n,alfk,xnew,fnew,m,ffunc,ctrl,fn_val)
    ...
    IF(-1 == lctrl) fn_val=0.5D0*dnrm2(m,fnew,1)**2
    !      write(10,*) 'FSUMSQ when ctrl=-1', fsumsq
    IF(lctrl < -10) ctrl=lctrl
    RETURN

20  IF(lctrl == 1) fn_val=0.5D0*dnrm2(m,fnew,1)**2
    !      write(10,*) 'FSUMSQ when ctrl= 1', fsumsq
    ctrl=lctrl
...
120    qraux(l) = zero
       ...
       DO  j = lp1, p
          ...
          tt = MAX(tt,zero)
          t = tt
          tt = one + 0.05D0*tt*(qraux(j)/work(j))**2
          IF (tt /= one) THEN
