...
REAL (dp), PARAMETER  :: a(12) = (/ -.7031250000000000D-01, .1121520996093750_dp,  &
      -.5725014209747314_dp, .6074042001273483D+01, -.1100171402692467D+03,  &
       .3038090510922384D+04, -.1188384262567832D+06, .6252951493434797D+07,  &
      -.4259392165047669D+09, .3646840080706556D+11, -.3833534661393944D+13,  &
       .4854014686852901D+15 /)
REAL (dp), PARAMETER  :: b(12) = (/ .7324218750000000D-01, -.2271080017089844_dp,  &
       .1727727502584457D+01, -.2438052969955606D+02, .5513358961220206D+03,  &
      -.1825775547429318D+05, .8328593040162893D+06, -.5006958953198893D+08,  &
       .3836255180230433D+10, -.3649010818849833D+12, .4218971570284096D+14,  &
      -.5827244631566907D+16 /)
REAL (dp), PARAMETER  :: a1(12) = (/ .1171875000000000_dp, -.1441955566406250_dp,  &
       .6765925884246826_dp, -.6883914268109947D+01, .1215978918765359D+03,  &
      -.3302272294480852D+04, .1276412726461746D+06, -.6656367718817688D+07,  &
       .4502786003050393D+09, -.3833857520742790D+11, .4011838599133198D+13,  &
      -.5060568503314727D+15 /)
REAL (dp), PARAMETER  :: b1(12) = (/ -.1025390625000000_dp, .2775764465332031_dp,  &
      -.1993531733751297D+01, .2724882731126854D+02, -.6038440767050702D+03,  &
       .1971837591223663D+05, -.8902978767070678D+06, .5310411010968522D+08,  &
      -.4043620325107754D+10, .3827011346598605D+12, -.4406481417852278D+14,  &
       .6065091351222699D+16 /)

REAL (dp), PARAMETER  :: pi = 3.141592653589793_dp, rp2 = 0.63661977236758_dp
REAL (dp)  :: cs0, cs1, cu, ec, p0, p1, q0, q1, r, r0, r1, t1, t2, w0, w1, x2
INTEGER    :: k, k0
...
IF (x <= 12.0_dp) THEN
  ...
  DO  k = 1, 30
    w1 = w1 + 1.0_dp / k
    r1 = -0.25_dp * r1 / (k*(k+1)) * x2
    r = r1 * (2.0_dp*w1 + 1.0_dp/(k+1))
    cs1 = cs1 + r
    IF (ABS(r) < ABS(cs1)*1.0D-15) EXIT
  END DO
  by1 = rp2 * (ec*bj1 - 1.0_dp/x - 0.25_dp*x*cs1)
ELSE
  k0 = 12
  IF (x >= 35.0) k0 = 10
  IF (x >= 50.0) k0 = 8
...
REAL (dp)  :: x, bj0, by0, dj0, dy0, bj1, by1, dj1, dy1

WRITE (*,*) 'Please enter x '
READ (*,*) x
WRITE (*,5100) x
WRITE (*,*) '  x          J0(x)          J0''(X)         J1(X)          J1''(X)'
WRITE (*,*) '----------------------------------------------------------------------'
CALL jy01a(x, bj0, dj0, bj1, dj1, by0, dy0, by1, dy1)
WRITE (*,5000) x, bj0, dj0, bj1, dj1
WRITE (*,*)
WRITE (*,*) '  x          Y0(x)          Y0''(X)         Y1(X)          Y1''(X)'
WRITE (*,*) '----------------------------------------------------------------------'
WRITE (*,5000) x, by0, dy0, by1, dy1
STOP
