...
SUBROUTINE tn (ierror, n, x, f, g, sfun)
...
! WHERE X IS A VECTOR OF N REAL VARIABLES.  THE METHOD USED IS A TRUNCATED-
! NEWTON ALGORITHM (SEE "NEWTON-TYPE MINIMIZATION VIA THE LANCZOS METHOD"
! BY S.G. NASH (SIAM J. NUMER. ANAL. 21 (1984), PP. 770-778).
! THIS ALGORITHM FINDS A LOCAL MINIMUM OF F(X).  IT DOES NOT ASSUME THAT THE
! FUNCTION F IS CONVEX (AND SO CANNOT GUARANTEE A GLOBAL SOLUTION), BUT DOES
! ASSUME THAT THE FUNCTION IS BOUNDED BELOW.  IT CAN SOLVE PROBLEMS HAVING ANY
! NUMBER OF VARIABLES, BUT IT IS ESPECIALLY USEFUL WHEN THE NUMBER OF
! VARIABLES (N) IS LARGE.
...
! THIS IS AN EASY-TO-USE DRIVER FOR THE MAIN OPTIMIZATION ROUTINE LMQN.
! MORE EXPERIENCED USERS WHO WISH TO CUSTOMIZE PERFORMANCE OF THIS ALGORITHM
! SHOULD CALL LMQN DIRECTLY.

!----------------------------------------------------------------------
! THIS ROUTINE SETS UP ALL THE PARAMETERS FOR THE TRUNCATED-NEWTON ALGORITHM.
! THE PARAMETERS ARE:
...
INTEGER    :: i, maxfun, maxit, msglvl, nmax
! EXTERNAL         sfun
...
810 FORMAT(//,' OPTIMAL FUNCTION VALUE = ', g22.15)
820 FORMAT(t11, 'CURRENT SOLUTION IS (AT MOST 10 COMPONENTS)' /  &
           t15, 'I           X(I)')
830 FORMAT(t11, i5, '  ', g22.15)
END SUBROUTINE tn



SUBROUTINE tnbc (ierror, n, x, f, g, sfun, low, up, ipivot)
...
! ETA    - SEVERITY OF THE LINE SEARCH
! MAXFUN - MAXIMUM ALLOWABLE NUMBER OF FUNCTION EVALUATIONS
! XTOL   - DESIRED ACCURACY FOR THE SOLUTION X*
! STEPMX - MAXIMUM ALLOWABLE STEP IN THE LINESEARCH
! ACCRCY - ACCURACY OF COMPUTED FUNCTION VALUES
...
SUBROUTINE lmqn (ifail, n, x, f, g, sfun, msglvl, maxit, maxfun, eta, &
                 stepmx, accrcy, xtol)
...
IF (nwhy < 0) GO TO 120
CALL setucr(small,nftotl,niter,n,f,fnew,fm,gtg,oldf,sfun,g,x)
...
modet = msglvl - 3
CALL modlnp(modet,zsol,gv,r,v, diagb,emat,x,g,  &
            zk, n,niter,maxit,nfeval,nmodif, nlincg,upd1,yksk,  &
            gsk,yrsr,lreset,sfun,.false.,ipivot, accrcy,gtpnew,gnorm,xnorm)
...
SUBROUTINE lmqnbc (ifail, n, x, f, g, sfun, low, up, ipivot, msglvl, &
...
CALL modlnp(modet, zsol, gv, r, v, diagb, emat, x, g, &
            ...
            yksk, gsk, yrsr, lreset, sfun, .true., ipivot, accrcy, gtpnew,  &
            gnorm, xnorm)
...
CALL modlnp(modet,zsol,gv,r,v, diagb,emat,x,g,  &
            zk, n,niter,maxit,nfeval,nmodif, nlincg,upd1,yksk,  &
            gsk,yrsr,lreset,sfun,.true.,ipivot, accrcy,gtpnew,gnorm,xnorm)
IF (newcon) GO TO 20
IF (lreset) GO TO 110
...
SUBROUTINE modlnp(modet, zsol, gv, r, v, diagb, emat, x, g, zk, n,  &
                  niter, maxit, nfeval, nmodif, nlincg, upd1, yksk, gsk,   &
                  yrsr, lreset, sfun, bounds, ipivot, accrcy, gtp, gnorm,  &
                  xnorm)
...
! CG ITERATION TO SOLVE SYSTEM OF EQUATIONS
  ...
  CALL gtims(v,gv,n,x,g,sfun,first,delta,accrcy,xnorm)
...
SUBROUTINE gtims(v, gv, n, x, g, sfun, first, delta, accrcy, xnorm)
...
SUBROUTINE linder(n, sfun, small, epsmch, reltol, abstol, tnytol, eta,  &
