...
REAL (dpr), INTENT(IN OUT)  :: v
REAL (dpr), INTENT(IN)      :: x
REAL (dpr), INTENT(OUT)     :: dv(0:)
REAL (dpr), INTENT(OUT)     :: dp(0:)
REAL (dpr), INTENT(OUT)     :: pdf
REAL (dpr), INTENT(OUT)     :: pdd

REAL (dpr)  :: ep, f, f0, f1, pd, pd0, pd1, s0, v0, v1, v2, vh, xa
...
REAL (dpr), INTENT(IN)   :: va
REAL (dpr), INTENT(IN)   :: x
REAL (dpr), INTENT(OUT)  :: pd

REAL (dpr)  :: a0, ep, eps, g0, g1, ga0, gm, pi, r, r1,  sq2, va0, vm, vt
INTEGER     :: m

eps = 1.0D-15
pi = 3.141592653589793_dpr
sq2 = SQRT(2.0D0)
ep = EXP(-.25D0*x*x)
va0 = 0.5D0 * (1.0D0-va)
IF (va == 0.0) THEN
...
REAL (dpr), PARAMETER  :: pi = 3.141592653589793_dpr
REAL (dpr)  :: a0, ep, eps, gl, r, vl, x1
INTEGER     :: k
...
REAL (dpr), INTENT(IN)   :: va
REAL (dpr), INTENT(IN)   :: x
REAL (dpr), INTENT(OUT)  :: pv

REAL (dpr), PARAMETER  :: pi = 3.141592653589793_dpr
REAL (dpr)  :: a0, dsl, eps, gl, pdl, qe, r, x1
INTEGER     :: k
...
REAL (dpr), INTENT(IN)   :: x
REAL (dpr), INTENT(OUT)  :: ga

REAL (dpr), PARAMETER  :: g(26) = (/  &
       1.0_dpr, 0.5772156649015329_dpr, -0.6558780715202538_dpr,  &
      -0.420026350340952D-1, 0.1665386113822915_dpr,   &
...
ELSE
  ...
  IF (ABS(x) > 1.0_dpr) THEN
    ga = ga * r
    IF (x < 0.0_dpr) ga = -pi / (x*ga*SIN(pi*x))
  END IF
END IF
RETURN
END SUBROUTINE gamma

END MODULE pbdv_func
...
!             Dv(x) and their derivatives using subroutine PBDV
!    Input:   x --- Argument of Dv(x)
!             v --- Order of Dv(x)
!    Output:  DV(na) --- Dn+v0(x)
!             DP(na) --- Dn+v0'(x)
...
REAL (dpr)  :: dv(0:100), dp(0:100), pdd, pdf, v, v0, vk, x
INTEGER     :: k, na, nv
...
DO  k = 0, na
  vk = k * SIGN(1,nv) + v0
  WRITE (*,5100) vk, dv(k), dp(k)
END DO
WRITE (*,*)
WRITE (*,5200) v, pdf, pdd
STOP

5000 FORMAT (' v =', f6.2,',    x =', f6.2)
5100 FORMAT (' ', f5.1, 2g16.8)
5200 FORMAT (' v =', f5.1, ',  Dv(x)=', g14.8,',   Dv''(X)=', g14.8)
END PROGRAM mpbdv
