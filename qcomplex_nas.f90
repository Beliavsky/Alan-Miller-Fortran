...
! Quadruple-precision complex conjugate
...
! Local variables
TYPE (quad)            :: r
REAL (r10), PARAMETER  :: vsmall = TINY(1.0_r10)
...
IF (x%qr%hi < 0.0_r10) THEN
  r = z%qr
  z%qr = z%qi
  z%qi = r
END IF
IF (x%qi%hi < 0.0_r10) z%qi = - z%qi
...
IF ( ABS(x%qr%hi) > ABS(x%qi%hi) ) THEN
  a = x%qi / x%qr
  z = ABS( x%qr ) * SQRT( quad(1.0_r10, 0.0_r10) + a**2 )
ELSE IF ( x%qi%hi == 0.0_r10 ) THEN
  z = 0.0_r10
ELSE
  a = x%qr / x%qi
  z = ABS( x%qi ) * SQRT( quad(1.0_r10, 0.0_r10) + a**2 )
END IF
