...
REAL (dp), INTENT(IN)      :: x
REAL (dp), INTENT(IN)      :: y
COMPLEX (dp), INTENT(OUT)  :: cqm(0:mm,0:n)
COMPLEX (dp), INTENT(OUT)  :: cqd(0:mm,0:n)

COMPLEX (dp)  :: cq0, cq1, cq10, cqf, cqf0, cqf1, cqf2, z, zq, zs
REAL (dp)     :: xc
INTEGER       :: i, j, k, km, ls
...
END IF
xc = ABS(z)
IF (AIMAG(z) == 0.0D0 .OR. xc < 1.0D0) ls = 1
IF (xc > 1.0D0) ls = -1
...
IF (xc < 1.0001D0) THEN
  ...
  DO  i = 0, 1
    DO  j = 2, n
      cqm(i,j) = ((2*j-1)*z*cqm(i,j-1) - (j+i-1)*cqm(i,j-2)) / (j-i)
    END DO
  END DO
  DO  j = 0, n
    DO  i = 2, m
      cqm(i,j) = -2 * (i-1) * z / zq * cqm(i-1,j) - ls * (j+i-1) * (j-i+2) * cqm(i-2,j)
    END DO
  END DO
ELSE
  IF (xc > 1.1) THEN
  ...
  cqf1 = 1.0D0
  DO  k = km, 0, -1
    cqf0 = ((2*k+3)*z*cqf1 - (k+1)*cqf2) / (k+2)
    IF (k <= n) cqm(1,k) = cqf0
    cqf2 = cqf1
    cqf1 = cqf0
  END DO
  ...
  DO  j = 0, n
    ...
    cq1 = cqm(1,j)
    DO  i = 0, m - 2
      cqf = -2 * (i+1) * z / zq * cq1 + (j-i) * (j+i+1) * cq0
      cqm(i+2,j) = cqf
      cq0 = cq1
      cq1 = cqf
    END DO
...
DO  j = 0, n
  DO  i = 1, m
    cqd(i,j) = ls * i * z / zs * cqm(i,j) + (i+j) * (j-i+1) / zq * cqm(i-1,j)
  END DO
END DO
RETURN
END SUBROUTINE clqmn
 
END MODULE clqmn_func
...
COMPLEX (dp)  :: cqm(0:40,0:40), cqd(0:40,0:40)
REAL (dp)     :: x, y
INTEGER       :: j, m, n
...
WRITE (*,*) '   m   n   Re[Qmn(z)]    Im[Qmn(z)]    Re[Qmn''(Z)]   IM[QMN''(Z)]'
WRITE (*,*) ' -----------------------------------------------------------------'
DO  j = 0, n
  WRITE (*,5000) m, j, cqm(m,j), cqd(m,j)
END DO
STOP

5000 FORMAT (' ', 2I4, 2g14.6, ' ', 2g14.6)
5100 FORMAT (' m =', i2, ', n =', i2, ', x =', f4.1, ', y =', f4.1)
END PROGRAM mclqmn
