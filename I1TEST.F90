...
! Date: 2003-01-22  Time: 10:17:52
 
!------------------------------------------------------------------
! FORTRAN 90 program to test BESI1, BESEI1

!  Method:

!     Accuracy tests compare function values against values generated with
!     the multiplication formula for small arguments and values generated from
!     a Taylor's Series Expansion using Amos' Ratio Scheme for initial values
!     for large arguments.
...
!         the floating-point arithmetic system.
!         Note that the call to MACHAR can be deleted provided the following
!         five parameters are assigned the values indicated
...
INTEGER    :: i, ibeta, iexp, ii, ind, irnd, it, j, j1, j2, k1, k2, k3,   &
              machep, maxexp, mb, mborg, minexp, mb2, n, negep, ngrd
REAL (dp)  :: a, ait, ak, akk, albeta, b, beta, const, d, del, delta,  &
              e, eps, epsneg, f, ovrchk, r6, r7, sum, temp, t1, t2, w, x,  &
              xa, xb, xbad, xj1, xl, xmax, xmb, xmin, xn, x1, y, z, zz
REAL (dp)  :: u(560), u2(560)
REAL (dp)  :: xlarge = 1.0D4
REAL (dp), PARAMETER  :: zero = 0.0_dp, half = 0.5_dp, one = 1.0_dp,  &
           three = 3.0_dp, sixten = 16.0_dp, hund = 100.0_dp, &
           x99 = -999.0_dp, xlam = 1.03125_dp, c = 0.9189385332_dp
REAL (dp), PARAMETER  :: ARR(8,7) = RESHAPE(  &
           (/ 1.0D0, -1.0D0, 1.0D0, -2.0D0, 1.0D0, -3.0D0, 1.0D0, -4.0D0, &
...
STOP
!-----------------------------------------------------------------
5000 FORMAT ('1Test of I1(X) vs Multiplication Theorem'//)
5100 FORMAT ('1Test of I1(X) vs Taylor series'//)
5200 FORMAT (i7,' Random arguments were tested from the interval ','(',  &
    f5.2,',',f5.2,')'//)
5300 FORMAT (' I1(X) was larger', i6,' times,'/ t11, ' agreed', i6,  &
...
6000 FORMAT (' I1(XMIN) = ',e24.17/)
6100 FORMAT (' I1(',i1,') = ',e24.17/)
6200 FORMAT (' I1(',e24.17,' ) = ',e24.17/)
6300 FORMAT (' E**-X * I1(XMAX) = ',e24.17/)
6400 FORMAT (' Tests near the largest argument for unscaled functions'/ )
6500 FORMAT (' This concludes the tests.')
!     ---------- Last line of BESI1 test program ----------
END PROGRAM i1test
...
REAL (dp), PARAMETER   :: one = 1.0_dp, eight = 8.0_dp, three = 3.0_dp,  &
                          half = 0.5_dp

fn_val = x - half * LOG(x) + LOG(one - three/(eight*x))
RETURN
END FUNCTION top
...
fn_val = three / ((eight*x - three)*x) + one - half / x
RETURN
END FUNCTION bot
