...
!--------------------------------------------------------------------
!  FORTRAN 90 program to test BESK0
...
!              MACHAR can be deleted provided the following three parameters
...
REAL (dp)  :: a, ait, albeta, amaxexp, b, beta, c, const, del, eps, &
              epsneg, r6, r7, sum, t, w, x, xa, xb, xl, xlam, xlarge, xmax,  &
              xmb, xmin, xn, x1, y, z, zz
REAL (dp)  :: u(0:559)
REAL (dp), PARAMETER  :: zero = 0.0_dp, half = 0.5_dp, one = 1.0_dp,   &
                         eight = 8.0_dp, xnine = 9.0_dp, twenty = 20.0_dp,  &
                         five = 5.0_dp, oneneg = -1.0_dp, xden = 16.0_dp,   &
                         all9 = -999.0_dp, pi = 3.141592653589793_dp
...
STOP
!-----------------------------------------------------------------
5000 FORMAT ('1Test of K0(X) vs Multiplication Theorem'//)
5100 FORMAT (i7, ' random arguments were tested from the interval (',  &
    f5.1, ',', f5.1, ')'//)
5200 FORMAT (' ABS(K0(X)) was larger', i6, ' times,'/ t21,' agreed',i6,  &
...
6100 FORMAT (' K0(XMIN) = ',e24.17/)
6200 FORMAT (' K0(',i1,') = ',e24.17/)
6300 FORMAT (' K0(',e24.17,' ) = ',e24.17/)
6400 FORMAT (' E**X * K0(XMAX) = ',e24.17/)
...
REAL (dp), PARAMETER   :: one = 1.0_dp, eight = 8.0_dp, one28 = 128.0_dp, &
                          half = 0.5_dp, xnine = 9.0_dp, two = 2.0_dp

fn_val = -x - half * LOG(two*x) + LOG(one - (one/eight-xnine/one28/x)/x)
RETURN
END FUNCTION top
...
REAL (dp), PARAMETER   :: one = 1.0_dp, one28 = 128.0_dp, xden = 16.0_dp,  &
                          xnine = 9.0_dp, half = 0.5_dp, ateten = 18.0_dp

fn_val = (xden*x - ateten) / (((one28*x-xden)*x + xnine)*x) - one - half / x
RETURN
END FUNCTION bot
