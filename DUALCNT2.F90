...
! The penalty coefficient is chosen automatically by the solver.
...
! EXTERNAL dualshel,dualobjf,dualcntf,null
...
! Analytical gradients of the objective function are not supplied:
flg=.false.

! Constrained problem:
flfc=.true.

! Analytical gradients of the constraints are not supplied:
flgc=.false.
...
! Call the solver:
CALL solvopt(n, x, f, dualobjf, flg, null, options, flfc, flgc, func=dualcntf)
