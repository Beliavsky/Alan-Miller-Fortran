...
REAL (dp), PARAMETER  :: srd = 57.29577951308233D0, eps = 1.0D-15,  &
                         pi = 3.141592653589793D0, pp2 = 1.2533141373155D0, &
                         p2p = .7978845608028654D0
REAL (dp)  :: c1, cs, fi0, s1, ss,   &
              x2, x4, xa, xc, xf, xf0, xf1, xg, xp, xq, xq2, xr, xs, xsu, xw
INTEGER    :: k, m
...
ELSE
  ...
  ELSE
    ...
    xf = 1.0D0
    DO  k = 1, 12
      xr = -.25D0 * xr * (4*k-1) * (4*k-3) / x4
      xf = xf + xr
    END DO
    xr = 1.0D0 / (2.0D0*xa*xa)
    xg = xr
    DO  k = 1, 12
      xr = -.25D0 * xr * (4*k+1) * (4*k-1) / x4
      xg = xg + xr
    END DO
...
REAL (dp)  :: x, fr, fi, fm, fa, gr, gi, gm, ga

DO
  WRITE (*,*) 'Please enter x: '
  READ (*,*) x
  WRITE (*,*)
  WRITE (*,*) '   x      Re[Fñ(x)]    ñIm[Fñ(x)]     Mod[Fñ(x)]   ñArg[Fñ(x)]'
  WRITE (*,*) ' --------------------------------------------------------------'
  CALL ffk(0, x, fr, fi, fm, fa, gr, gi, gm, ga)
  WRITE (*,5000) x, fr, fi, fm, fa
  WRITE (*,*)
  WRITE (*,*) '   x      Re[Kñ(x)]    ñIm[Kñ(x)]     Mod[Kñ(x)]   ñArg[Kñ(x)]'
  WRITE (*,*) ' --------------------------------------------------------------'
  WRITE (*,5000) x, gr, gi, gm, ga
END DO
STOP

5000 FORMAT (' ', f5.1, 3F14.8, f14.6)
END PROGRAM mffk
