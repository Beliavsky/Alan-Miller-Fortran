...
!----------------------------------------------------------------------
! FORTRAN 90 program to test RYBESL
...
REAL (dp)  :: a, ait, albeta, alpha, alphsq, a1, b, beta, d, del, delta,   &
              deriv, eps, epsneg, f, fxmx, r6, r7, sum, t1, t2,   &
              w, x, xj1, xl, xmax, xmb, xmin, xn, x1, y, ysq, z, zz
REAL (dp)  :: g(5), u(20), u2(20)
REAL (dp), PARAMETER  :: zero = 0.0_dp, half = 0.5_dp, one = 1.0_dp,  &
           onep25 = 1.25_dp, two = 2.0_dp, p875 = 0.875_dp, ten = 10.0_dp,  &
           sixten = 16.0_dp, x99 = -999.0_dp, xlam = 1.03125_dp,   &
           twobpi = 0.6366_dp
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
      IF (SIGN(one,u(1))*SIGN(one,u2(1)) < zero) GO TO 10
      d = -f * y
      mb = ncalc - 1
      xmb = mb
      sum = u2(mb+1)
...
!----------------------------------------------------------------------
      d = ABS(zz/u2(ind))
      IF ((d > onep25) .OR. (d < p875)) GO TO 10
      zz = zz * xlam ** alpha
    ELSE
!----------------------------------------------------------------------
!   Accuracy test is based on local Taylor's series expansion.
!   First, filter out cases where function values or derivatives are small.
!----------------------------------------------------------------------
      w = MIN(ABS(u(1)), ABS(u2(1)), ABS(u2(2)))
      IF (w < SQRT(twobpi/x)/sixten) GO TO 10
...
!----------------------------------------------------------------------
  ...
  IF (n /= 2000) WRITE (iout,5400) 2000 - n
  ...
  ELSE
    w = x99
  END IF
  WRITE (iout,5600) r6, ibeta, w, x1, a1
  WRITE (iout,5900) fxmx
  w = MAX(ait+w,zero)
...
mb = 5
CALL rybesl(x,alpha,mb,u,ncalc)
WRITE (iout,6100) x, alpha, mb, u(1), ncalc
alpha = half
mb = -mb
CALL rybesl(x,alpha,mb,u,ncalc)
WRITE (iout,6100) x, alpha, mb, u(1), ncalc
...
mb = 2
CALL rybesl(x,alpha,mb,u,ncalc)
WRITE (iout,6200) x, alpha
WRITE (iout,6400) ncalc, u(1)
x = x * sixten
mb = 2
CALL rybesl(x,alpha,mb,u,ncalc)
WRITE (iout,6200) x, alpha
...
STOP
!----------------------------------------------------------------------
5000 FORMAT ('1Test of Y(X,ALPHA) vs Multiplication Theorem'//)
5100 FORMAT ('1Test of Y(X,ALPHA) vs Taylor series'//)
5200 FORMAT (i7,' Random arguments were tested from the interval (',  &
    f5.2,',',f5.2,')'//)
5300 FORMAT (' Y(X,ALPHA) was larger', i6, ' times,'/ t16, ' agreed', i6,  &
    ' times, and'/ t12, 'was smaller', i6, ' times.'//)
5400 FORMAT (' NOTE: first ',i3,' arguments in test interval skipped'/  &
    t8, 'because multiplication theorem did not converge'//)
...
5800 FORMAT (' The root mean square relative error was', e15.4,' = ',i4,  &
    ' **',f7.2)
5900 FORMAT (t5, 'with Y(X,ALPHA) = ', e13.6)
6000 FORMAT ('1Check of Error Returns'///  &
    ' The following summarizes calls with indicated parameters'//  &
    ' NCALC different from MB indicates some form of error'//  &
    ' See documentation for RYBESL for details'// t8, 'ARG', t23,  &
    'ALPHA      MB      B(1)      NCALC'//)
6100 FORMAT (2E15.7, i5, e15.7, i5//)
6200 FORMAT (' RYBESL will be called with the arguments',2E13.6)
6300 FORMAT (' This should trigger an error message.')
6400 FORMAT (' NCALC returned the value',i5/  &
    ' and RYBESL returned U(1) = ',e13.6/)
6500 FORMAT (' Tests near the largest acceptable argument for RYBESL'/)
6600 FORMAT (' This concludes the tests.')
!     ---------- Last line of RYBESL test program ----------
END PROGRAM rytest
