...
REAL (dp), INTENT(IN)   :: q
REAL (dp), INTENT(IN)   :: x
REAL (dp), INTENT(OUT)  :: csf
REAL (dp), INTENT(OUT)  :: csd

REAL (dp)  :: a, eps, fg(251), qm, rd, xr
INTEGER    :: ic, k, kd, km
...
DO  k = 1, km
  ...
  ELSE IF (kd == 4) THEN
    csd = csd + 2.0_dp * k * fg(k) * COS(2*k*xr)
  END IF
...
WRITE (*,5100) x, csf, csd
STOP

5000 FORMAT (' KF =', i2, ',  m =', i2, ',  q =', f5.1, ',  x =', f5.1)
5100 FORMAT (t3, f5.1, 2F16.9)
END PROGRAM mmtu0
