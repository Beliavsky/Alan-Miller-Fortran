...
hm1 = .5 * q / (m*m-1.0)
hm3 = .25 * hm1 ** 3 / (m*m-4)
hm5 = hm1 * hm3 * q / ((m*m-1)*(m*m-9))
a0 = m * m + q * (hm1 + (5*m*m + 7)*hm3 + (9*m**4 + 58*m*m + 29)* hm5)
RETURN
END SUBROUTINE cvqm
 
END MODULE fcoef_func
...
REAL (dp)  :: a, fc(251), fb(251), q
INTEGER    :: kd, kf, ki, ks, l, m
...
DO  l = 1, m / 2 + 8
  IF (m == 2*INT(m/2)) THEN
    IF (m == 0) WRITE (*,5100) 2 * l - 2, fb(l)
    IF (m > 0 .AND. l == 1) WRITE (*,5100) 2 * l - 2, fb(l)
    IF (m > 0 .AND. l /= 1) WRITE (*,5100) 2 * l - 2, fb(l), fc(l-1)
  ELSE
    WRITE (*,5100) 2 * l - 1, fb(l), fc(l)
  END IF
END DO
STOP

5000 FORMAT (' m=', i3, ',    q=', f6.2)
5100 FORMAT (' ', i3, 2g18.8)
5200 FORMAT (' Expansion coefficients of Mathieu functions')
5300 FORMAT ('   n         Amn(q)            Bmn(q)')
END PROGRAM mfcoef
