...
IF (t > nin) THEN
  ...
  IF (ier /= 0) WRITE(*, *) 'Error IER = ', ier, ' from VMOVE'
END IF
...
PROGRAM t_nnls
! Generate artificial 300 x 17 matrices to test nonnegls and to compare
! it with NNLS
...
REAL (lsq_kind)            :: x(300,17), y(300), b(17), xx(17), w(300), rnorm
INTEGER                    :: i, j, nin, indx(17), mode, repl, t1(8), t2(8)
INTEGER, ALLOCATABLE       :: seed(:)
INTEGER                    :: k
REAL                       :: tlh, tajm
REAL (lsq_kind), PARAMETER :: wt = 1.0_lsq_kind
...
DO repl = 1, 200
  DO i = 1, 300
    ...
    DO j = 2, 17
      x(i,j) = 0.9*x(i,j-1) + x(i,j)
    END DO
    y(i) = y(i) - 0.5 + 0.25*( x(i,5) + x(i,10) + x(i,15) )
  END DO

  CALL DATE_AND_TIME(VALUES=t1)
  CALL startup(17, .FALSE.)
  DO i = 1, 300
    xx = x(i,:)
    CALL includ(wt, xx, y(i))
  END DO

  CALL nonnegls(nin, b)

  CALL DATE_AND_TIME(VALUES=t2)
  tajm = tajm + 60.*(t2(6)-t1(6)) + (t2(7)-t1(7)) + 0.001*(t2(8)-t1(8))
  WRITE(9, '(a/ (6g13.5))')  &
        ' Regression coefficients from NONNEGLS = ', b(1:nin)
  WRITE(9, '(a, g13.5)') 'SSERR = ', sserr

  CALL DATE_AND_TIME(VALUES=t1)
  CALL nnls (x, 300, 17, y, b, rnorm, w, indx, mode)
  CALL DATE_AND_TIME(VALUES=t2)
  tlh = tlh + 60.*(t2(6)-t1(6)) + (t2(7)-t1(7)) + 0.001*(t2(8)-t1(8))
  WRITE(9, '(a/ (6g13.5))')  &
        ' Regression coefficients from NNLS = ', b(indx(1:nin))
  WRITE(9, '(a, g13.5, a, i6, a, g13.5)')  &
           'RNORM = ', rnorm, '  MODE = ', mode, '  W(1) = ', w(1)
END DO

WRITE(*, '(a, f7.2, a, f7.2)') ' nonnegls time =', tajm, '   NNLS time =', tlh
