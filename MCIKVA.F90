...
COMPLEX (dp)  :: ca, ca1, ca2, cb, cbi0, cbk0, cbk1, cf, cf1, cf2, cg0, cg1, &
                 cgk, ci, ci0, cp, cr, cr1, cr2, cs, csu, ct, cvk, z1, z2
REAL (dp)     :: a0, gan, gap, piv, v0, v0n, v0p, vt, w0, ws, ws0
INTEGER       :: k, k0, m, n
...
ELSE
  ...
  cbk0 = cb * cs
END IF
cbk1 = (1.0_dp/z1 - cbi(1)*cbk0) / cbi(0)
cbk(0) = cbk0
cbk(1) = cbk1
cg0 = cbk0
cg1 = cbk1
DO  k = 2, n
  cgk = 2.0_dp * (v0+k-1) / z1 * cg1 + cg0
  cbk(k) = cgk
  cg0 = cg1
  cg1 = cgk
END DO
...
! N.B. COMMON has been changed to COMPLEX (dp).
...
5100 FORMAT (t9, 'v= n+v0,   v0 =', f7.2, ',   z =', f6.1, '+ i', f6 .1)
END PROGRAM mcikva
