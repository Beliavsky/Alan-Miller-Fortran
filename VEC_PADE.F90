...
!      ALGORITHM 766, COLLECTED ALGORITHMS FROM ACM.
!      THIS WORK PUBLISHED IN TRANSACTIONS ON MATHEMATICAL SOFTWARE,
!      VOL. 23, NO. 1, March, 1997, P.  91--110.

IMPLICIT NONE
public  :: VECTOR_PADE, build_T, build_T_star, build_S, build_S_star,  &
           mult_S, mult_S_star, scale_S, scale_S_star

integer, parameter, public  :: dp = SELECTED_REAL_KIND(12, 60)

!        Automatic arrays for VECTOR_PADE.
!        Variables used to compute NPHS.
         real (dp), dimension(:,:,:),  allocatable :: S_hat, New_S
         real (dp), dimension(:,:),    allocatable :: T
!        Variables used to compute NSPS.
         real (dp), dimension(:,:,:),  allocatable :: S_star_hat, New_S_star
         real (dp), dimension(:,:,:),  allocatable :: T_star
...
!   VECTOR_PADE subroutine parameters.
    ...
    real (dp),    dimension (0:,0:),    intent (in)     :: A
    real (dp),                          intent (in)     :: tau
    real (dp),    dimension (0:,0:,0:), intent (out)    :: S, S_star
    real (dp),    dimension (0:),       intent (out)    :: gamma,   &
                                                           gamma_star, kappa
...
!   Check the validity of input parameters.
    ...
    if (     k < 1                       .or. &
             k > size(n) - 1             .or. &
        norm_n > size(A(:,0)) - 1        .or. &
             k > size(A(0,:)) - 1        .or. &
           0.0 == A(0,0)                 .or. &
     maxval(n) > size(S(:,0,0)) - 2      .or. &
             k > size(S(0,:,0)) - 1      .or. &
             k > size(S(0,0,:)) - 1      .or. &
        norm_n > size(S_star(:,0,0)) - 1 .or. &
             k > size(S_star(0,:,0)) - 1 .or. &
             k > size(S_star(0,0,:)) - 1) flag = 3
...
!         current stable point m.
          ...
          do while (i+step<last_i .and. flag>0)
!         Inner loop
!         ************************************************************
...
!   build_T subroutine parameters.
    ...
    real (dp),    dimension (0:,0:),    intent (in)     :: A
    real (dp),    dimension (0:,0:,0:), intent (in)     :: S
    real (dp),    dimension (0:,0:),    intent (in out) :: T
...
subroutine build_T_star(A, S_star, k, m, prevnorm_nus, norm_nus, T_star)
...
!   build_T_star subroutine parameters.
    ...
    integer,      dimension (0:),       intent (in)     :: m
    real (dp),    dimension (0:,0:),    intent (in)     :: A
    real (dp),    dimension (0:,0:,0:), intent (in)     :: S_star
    real (dp),    dimension (0:,0:,1:), intent (in out) :: T_star
...
!   build_S subroutine parameters.
    ...
    integer,      dimension (0:),       intent (in)  :: n
    real (dp),    dimension (0:,0:),    intent (in)  :: A
    real (dp),    dimension (0:,0:,0:), intent (out) :: S
...
!   M is the striped Sylvester matrix associated with A.
    integer    :: ipvt(norm_n)
    real (dp)  :: M(norm_n, norm_n), B(norm_n)
...
!   build_S_star subroutine parameters.
    ...
    integer,      dimension (0:),       intent (in)  :: n
    real (dp),    dimension (0:,0:,1:), intent (in)  :: A_star
    real (dp),    dimension (0:,0:,0:), intent (out) :: S_star
    logical,                            intent (out) :: singular
...
!   M_star is the mosaic Sylvester matrix associated with A_star.
    ...
    real (dp)  :: M_star(knorm_n,knorm_n), B_star(knorm_n)
...
!      Reduce M_star into triangular form.
       call sgefa(M_star, knorm_n, ipvt, info)
...
!      Compute the first row of S_star.
       ...
       call sgesl(M_star, knorm_n, ipvt, B_star, 0)
       blockoffset = 0
       do beta=0, k
...
!   mult_S subroutine parameters.
    ...
    real (dp),    dimension (0:,0:,0:), intent (in)  :: S, S_hat
    real (dp),    dimension (0:,0:,0:), intent (out) :: New_S
...
subroutine mult_S_star(S_star, m, S_star_hat, nus, New_S_star, new_m, k)
...
!   mult_S_star subroutine parameters.
    integer,                            intent (in)  :: k
    integer,      dimension (0:),       intent (in)  :: m, nus, new_m
    real (dp),    dimension (0:,0:,0:), intent (in)  :: S_star, S_star_hat
    real (dp),    dimension (0:,0:,0:), intent (out) :: New_S_star
...
!   scale_S subroutine parameters.
    integer,                            intent (in)     :: k
    integer,      dimension (0:),       intent (in)     :: m
    real (dp),    dimension (0:,0:,0:), intent (in out) :: S
    real (dp),    dimension (0:),       intent (out)    :: gamma
...
!   scale_S_star subroutine parameters.
    integer,                            intent (in)     :: k
    integer,      dimension (0:),       intent (in)     :: m
    real (dp),    dimension (0:,0:,0:), intent (in out) :: S_star
    real (dp),    dimension (0:),       intent (out)    :: gamma_star
...
!********************************************************************
    ...
    integer, intent(in)   :: n(0:), last_i, i, k
...
! Subroutines used from Linpack

SUBROUTINE sgefa(a, n, ipvt, info)
 
! Code converted using TO_F90 by Alan Miller
! Date: 2000-04-09  Time: 23:27:18
...
!      info    integer
!              = 0  normal value.
!              = k  if  u(k,k) == 0.0 .  this is not an error condition for
!                   this subroutine, but it does indicate that sgesl or sgedi
!                   will divide by zero if called.  Use  rcond  in sgeco for a
...
SUBROUTINE sgesl(a, n, ipvt, b, job)
...
!         call sgeco(a, n, ipvt, rcond, z)
!         if (rcond is too small) go to ...
!         do 10 j = 1, p
!            call sgesl(a, n, ipvt, c(1:,j), 0)
!      10 continue
!
!   linpack. this version dated 08/14/78 .
!   cleve moler, university of new mexico, argonne national lab.
